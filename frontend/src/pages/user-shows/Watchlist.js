...
const WatchlistShows = () => {
    ...
    useEffect(() => {
        const fetchWatchlistShows = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/user/watchlist-shows/${username}`);
    ...
    const sortedShows = sortShows(watchlistShows, sortType, ascending);
